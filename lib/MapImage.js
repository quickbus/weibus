...
Label.prototype.toLabelStyleString = function(name) {
  return [name||this.name , 1, 15, this.color, this.backgroundColor,1].join();
};
...
MapImage.prototype.showMarker = function(marker) {
  ...
  var markerStyles = 'markerStyles=l, ,0x0000FF';
  var zoom = 'zoom=15';
  var size = 'height=300&width=600';
  return APIURL + [size,labelString, styleString, markers,markerStyles,zoom].join('&');
};
